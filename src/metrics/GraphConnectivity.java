...
public class GraphConnectivity {
			...
			for (Vertex v : G.getVertices()) {
				if (!alreadyin.contains(v.getId().toString())) {
					...
					while (!stack.isEmpty()) {
						...
						for (Vertex n : current.getVertices(Direction.BOTH,
								UserUtility.FRIEND)) {
							...
							if (newv.getId().toString()
									.compareTo(newn.getId().toString()) > 0)
								edge_id = newv.getId().toString() + "-"
										+ newn.getId().toString();
							else
								edge_id = newn.getId().toString() + "-"
										+ newv.getId().toString();
							try {
								newg.addEdge(edge_id, newv, newn,
										UserUtility.FRIEND);
							} catch (IllegalArgumentException e) { /*
																	 * do
																	 * nothing
	...
	public static class ConnectedComponents {
		private Map<Vertex, Integer> components; // marked[v] = has vertex v
													// been marked?
		private List<Integer> size; // size[id] = number of vertices in given
									// component
		...
		private void reachableNodes(Vertex v, String[] edge_labels) {
			...
			while (!stack.isEmpty()) {
				Vertex current = stack.pop();
				for (Vertex n : current
						.getVertices(Direction.BOTH, edge_labels)) {
		...
		public String summary(boolean details) {
			...
			sb.append("Connected components:").append(System.lineSeparator())
					.append("Component\t\tSize").append(System.lineSeparator());
			for (int i = 0; i < size.size(); ++i) {
				sb.append("Component ").append(i).append("\t\t")
						.append(size.get(i)).append(System.lineSeparator());
			}
			if (details) {
				sb.append(System.lineSeparator()).append("Vertex\tComponent")
						.append(System.lineSeparator());
				for (Map.Entry<Vertex, Integer> c : components.entrySet()) {
					sb.append(c.getKey().getId().toString()).append("\t\t")
							.append(c.getValue())
							.append(System.lineSeparator());
				}
			}
